...
async function getOwnedProjectRow(projectId, ownerId) {
    const db = getDb();
    const rows = await db
        .select()
        .from(projects)
...
async function getOwnedProjectKeyMap(ownerId) {
    const db = getDb();
    const rows = await db
...
export async function getProjectDashboard(projectId, ownerId) {
    ...
    const [statusRows, blockedRows, updatedRows, deployedRows, blockedTicketRows] = await Promise.all([
        db
            .select({ status: tickets.status, count: sql `count(*)` })
            .from(tickets)
            .where(active)
    ...
    for (const row of statusRows) {
        if (row.status === 'PLANNED')
            counts.planned = Number(row.count);
        else if (row.status === 'IN_PROGRESS')
            counts.inProgress = Number(row.count);
        else if (row.status === 'IMPLEMENTED')
            counts.implemented = Number(row.count);
        else if (row.status === 'TESTED')
            counts.tested = Number(row.count);
        else if (row.status === 'DEPLOYED')
            counts.deployed = Number(row.count);
    }
    return {
...
function dedupeByFurthestStage(inProgress, blocked, implemented, tested) {
    const stageRank = {
        in_progress: 0,
        blocked: 1,
    ...
    for (const t of inProgress)
        chosen.set(t.id, { stage: 'in_progress', ticket: t });
...
export async function getFocus(ownerId, projectId) {
    if (projectId)
    ...
    const chosen = dedupeByFurthestStage(inProgress.map((r) => r.ticket), blocked.map((r) => r.ticket), implemented.map((r) => r.ticket), tested.map((r) => r.ticket));
...
export async function getAccomplishments(projectId, ownerId, includeArchived = false) {
    await getOwnedProjectRow(projectId, ownerId);
    const db = getDb();
    ...
    for (const row of rows) {
        ...
        const deployed = new Date(`${row.ticket.deployedAt.slice(0, 10)}T${row.ticket.deployedAt.slice(11)}Z`);
        if (deployed >= startOfToday)
            groups[0].tickets.push(dto);
        else if (deployed >= startOfWeek)
            groups[1].tickets.push(dto);
        else if (deployed >= startOfMonth)
            groups[2].tickets.push(dto);
        else
            groups[3].tickets.push(dto);
    }
    return { groups };
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
export async function getTagsForTickets(ticketIds) {
