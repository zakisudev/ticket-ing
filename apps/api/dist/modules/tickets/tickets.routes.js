...
import { attachTagSchema, createChecklistItemSchema, createLinkSchema, createRelationSchema, createTagSchema, createTicketSchema, updateChecklistItemSchema, updateTicketSchema, ticketFilterParamsSchema, listSortFieldsSchema, } from '@zakisu-tickets/shared';
import { z } from 'zod';
...
function requireTicketId(req) {
    const id = req.params.ticketId;
    if (!id)
        throw ApiError.validation('Missing ticketId');
    return id;
}
function requireProjectIdParam(req) {
...
projectTicketsRouter.get('/:projectId/tickets', async (req, res, next) => {
    try {
        ...
        const sort = parseBody(z.object({ sortBy: listSortFieldsSchema.optional(), sortDir: z.enum(['asc', 'desc']).optional() }), req.query);
...
projectTicketsRouter.get('/:projectId/tickets/by-number/:ticketNumber', async (req, res, next) => {
    try {
        const ticket = await getTicketByNumber(requireProjectIdParam(req), req.user.id, req.params.ticketNumber ?? '');
        res.json({ ticket });
    }
    catch (err) {
        next(err);
