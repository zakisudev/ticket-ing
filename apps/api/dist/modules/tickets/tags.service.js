...
export async function listProjectTags(projectId, ownerId) {
...
export async function createTag(projectId, ownerId, input) {
    await getOwnedProject(projectId, ownerId);
    ...
    try {
        ...
        try {
            ...
            await conn.execute(`INSERT INTO tags (id, project_id, name, slug, color) VALUES (?, ?, ?, ?, ?)`, [id, projectId, input.name, slug, input.color ?? null]);
            await conn.commit();
            conn.release();
            released = true;
        }
        ...
        if (cause?.code === 'ER_DUP_ENTRY' || err.code === 'ER_DUP_ENTRY') {
...
export async function deleteTag(projectId, tagId, ownerId) {
    await getOwnedProject(projectId, ownerId);
    const db = getDb();
    const rows = await db
        .select()
...
export async function listTicketTags(ticketId, ownerId) {
...
export async function attachTag(ticketId, ownerId, tagId) {
    const { ticket } = await getOwnedTicketContextOr404(ticketId, ownerId);
    const db = getDb();
    ...
    try {
        ...
        await conn.execute(`INSERT INTO ticket_tags (ticket_id, tag_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE tag_id = tag_id`, [ticketId, tagId]);
...
export async function detachTag(ticketId, ownerId, tagId) {
    const { ticket } = await getOwnedTicketContextOr404(ticketId, ownerId);
    void ticket;
    const db = getDb();
