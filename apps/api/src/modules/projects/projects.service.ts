...
import type { CreateProjectInput, ProjectDto, UpdateProjectInput } from '@zakisu-tickets/shared';
...
function toDto(row: ProjectRow, ticketCount = 0): ProjectDto {
  return {
    id: row.id,
    name: row.name,
    slug: row.slug,
...
export async function getOwnedProjectOr404(
  projectId: string,
  ownerId: string
): Promise<ProjectRow> {
  const db = getDb();
...
async function ticketCountFor(projectId: string): Promise<number> {
  const db = getDb();
  const [row] = await db
    .select({ value: count() })
    .from(tickets)
...
export async function listProjects(ownerId: string, includeArchived: boolean): Promise<ProjectDto[]> {
  const db = getDb();
  const rows = await db
    .select()
    .from(projects)
...
export async function getProject(projectId: string, ownerId: string): Promise<ProjectDto> {
