...
type LinkRow = typeof ticketLinks.$inferSelect;

function toDto(row: LinkRow): TicketLinkDto {
  return {
    id: row.id,
 ...
 */
export function shortCommitHash(label: string | null): string | null {
  if (!label) return null;
  return /^[0-9a-f]{8,40}$/i.test(label.trim()) ? label.trim().slice(0, 7) : null;
}

export async function listLinks(ticketId: string, ownerId: string): Promise<TicketLinkDto[]> {
...
export async function addLink(
  ...
  try {
    ...
    await conn.execute(
      `INSERT INTO ticket_links (id, ticket_id, type, label, url) VALUES (?, ?, ?, ?, ?)`,
      [id, ticketId, input.type, input.label ?? null, input.url]
    );
...
export async function deleteLink(ticketId: string, linkId: string, ownerId: string): Promise<void> {
  await getOwnedTicketContextOr404(ticketId, ownerId);
  const db = getDb();
  const rows = await db
    .select()
