import { and, asc, desc, eq, inArray, isNull, isNotNull, or, like, sql, type SQL } from 'drizzle-orm';
...
// ---------------------------------------------------------------------------
const MILESTONE_FIELD: Partial<
  Record<TicketRow['status'], 'started_at' | 'implemented_at' | 'tested_at' | 'deployed_at'>
> = {
...
const TEXT_FIELD_MAP: Record<string, keyof UpdateTicketInput> = {
  summary: 'summary',
...
export async function updateTicket(
  ...
  try {
    ...
    const rawCol = <T>(name: string): T => raw[name] as T;
    ...
    if (input.isBlocked !== undefined) {
      if (input.isBlocked && !rawCol<boolean>('is_blocked')) {
      ...
      } else if (!input.isBlocked && rawCol<boolean>('is_blocked')) {
        // Preserve the previous reason in activity; clear the live field.
        const previousReason = rawCol<string | null>('blocked_reason');
        ...
        const reason = input.blockedReason?.trim();
        if (reason && reason !== rawCol<string>('blocked_reason')) {
    ...
    if (input.archived !== undefined) {
      const currentlyArchived = rawCol<Date | string | null>('archived_at') != null;
    ...
    for (const [column, inputKey] of Object.entries(TEXT_FIELD_MAP)) {
      ...
      const currentValue = rawCol<string | null>(column);
...
// ---------------------------------------------------------------------------
function buildFilterConditions(projectId: string, f: TicketFilterParams): SQL[] {
  ...
  if (f.type) conditions.push(eq(tickets.type, f.type));
  if (f.blocked) {
    conditions.push(eq(tickets.isBlocked, f.blocked === 'true'));
  }
  if (f.hasLimitations === 'true') conditions.push(isNotNull(tickets.limitations));
  if (f.hasKnownIssues === 'true') conditions.push(isNotNull(tickets.knownIssues));
  if (f.hasFollowUps === 'true') conditions.push(isNotNull(tickets.followUpNotes));

  // has*: 'false' variants negate.
  if (f.hasLimitations === 'false') conditions.push(isNull(tickets.limitations));
  if (f.hasKnownIssues === 'false') conditions.push(isNull(tickets.knownIssues));
  if (f.hasFollowUps === 'false') conditions.push(isNull(tickets.followUpNotes));
...
export async function searchTickets(
  ...
  sort: { sortBy?: ListSortField; sortDir?: 'asc' | 'desc' } = {}
