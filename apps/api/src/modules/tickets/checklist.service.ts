...
type ChecklistRow = typeof ticketChecklistItems.$inferSelect;
...
export async function listChecklist(ticketId: string, ownerId: string): Promise<ChecklistItemDto[]> {
...
async function getOwnedItem(
  ...
  itemId: string,
  ownerId: string
): Promise<{ row: ChecklistRow; ticketVersion: number }> {
...
export async function addChecklistItem(
  ...
  try {
    ...
    const [maxRows] = await conn.query<RowDataPacket[]>(
      `SELECT COALESCE(MAX(sort_order), -1) AS maxSort FROM ticket_checklist_items WHERE ticket_id = ?`,
      [ticketId]
    );
    const maxSort = Number((maxRows as { maxSort: number }[])[0].maxSort);
    const id = newId();
    await conn.execute(
      `INSERT INTO ticket_checklist_items (id, ticket_id, text, completed, sort_order) VALUES (?, ?, ?, FALSE, ?)`,
      [id, ticketId, input.text, maxSort + 1]
    );
...
export async function updateChecklistItem(
  ...
  try {
    ...
    const [currentRows] = await conn.execute(
      `SELECT * FROM ticket_checklist_items WHERE id = ? AND ticket_id = ? FOR UPDATE`,
      [itemId, ticketId]
    );
    const current = (currentRows as ChecklistRow[])[0];
    if (!current) throw ApiError.notFound('Checklist item not found');

    const sets: string[] = [];
    const params: unknown[] = [];
    const activity: { type: 'CHECKLIST_ITEM_COMPLETED' | 'CHECKLIST_ITEM_REOPENED' }[] = [];
    ...
    if (sets.length > 0) {
      await conn.query(
        `UPDATE ticket_checklist_items SET ${sets.join(', ')} WHERE id = ?`,
        [...params, itemId]
      );
    }
...
export async function reorderChecklistItem(
  ...
  try {
    ...
    const [rows] = await conn.query(
      `SELECT id, sort_order FROM ticket_checklist_items WHERE ticket_id = ? ORDER BY sort_order ASC, created_at ASC`,
      [ticketId]
    );
    const ordered = rows as { id: string; sort_order: number }[];
    const index = ordered.findIndex((r) => r.id === itemId);
    if (index === -1) throw ApiError.notFound('Checklist item not found');
